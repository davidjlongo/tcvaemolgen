...
def get_path_features(rd_mol, path_atoms, path_length, max_path_length,
    ...
    for path_idx in range(max_path_length):
        path_bond = path_bonds[path_idx] if path_idx < len(path_bonds) else None #TODO
        features.append(mol_features.get_path_bond_feature(path_bond))
        #print(f'{path_idx}/{len(features)}' + ':\t' + path_idx*'\t' + \
        #    str(len(mol_features.get_path_bond_feature(path_bond))))
    if  p_embed:

...
class DGLJTNNDecoder(nn.Module):
    def __init__(self, vocab, hidden_size, latent_size, embedding=None):
        ...
        self.dec_tree_edge_update = GRUUpdate(hidden_size).cuda()

        self.W = nn.Linear(latent_size + hidden_size, hidden_size).cuda()
        self.U = nn.Linear(latent_size + 2 * hidden_size, hidden_size).cuda()
        self.W_o = nn.Linear(hidden_size, self.vocab_size).cuda()
        self.U_s = nn.Linear(hidden_size, 1).cuda()

...
from .modules.jtnn_enc import DGLJTNNEncoder
from .modules.jtnn_dec import DGLJTNNDecoder
from .modules.gcn import GraphConvNet as DGLMPN
from .modules.gcn import mol2dgl_enc as mol2dgl_enc
from .modules.jtgcn import DGLJTMPN
from .modules.jtgcn import mol2dgl_single as mol2dgl_dec
...
class DGLJTNNVAE(nn.Module):
    ...
    def __init__(self, vocab, hidden_size, latent_size, depth, args=None):
        ...
        self.T_var = nn.Linear(hidden_size, latent_size // 2).cuda()
        self.G_mean = nn.Linear(hidden_size, latent_size // 2).cuda()
        self.G_var = nn.Linear(hidden_size, latent_size // 2).cuda()
    ...
    def forward(self, mol_batch, beta=0, e1=None, e2=None):
        ...
        """
        print("%8s %8f "*6 % (
            "Fwd:",encode_start-forward_start,
            "Enc:",sample_start-encode_start,
            "Smp:",decoder_start-sample_start,
            "Dec:",assm_start-decoder_start,
            "Assm:",stereo_start-assm_start,
            "Stereo:",loss_start-stereo_start
        ))
"""
